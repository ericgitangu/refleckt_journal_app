...
const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL || 'https://api.refleckt.app';
 ...
 */
export async function serverApiClient<T>(endpoint: string, token: string, options: Omit<ApiOptions, 'useAuth'> = {}): Promise<T> {
  try {
    const fetchOptions: RequestInit = {
      method: options.method || 'GET',
      headers: {
