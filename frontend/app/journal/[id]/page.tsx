    ...
    <div className="container mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 py-8">
      <div className="md:col-span-2">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="space-x-2">
            <Button onClick={handleSave} disabled={updateEntryMutation.isLoading}>
              {updateEntryMutation.isLoading ? 'Saving...' : 'Save'}
            </Button>
            <Button variant="destructive" onClick={handleDelete} disabled={deleteEntryMutation.isLoading}>
              Delete
            </Button>
          </div>
        </div>
      ...
      {settings?.ai_insights_enabled && (
        <div className="md:col-span-1">
          <InsightsPanel insights={insights} isLoading={insightsLoading} entryId={id as string} />
        </div>
      )}
    </div>
  );
