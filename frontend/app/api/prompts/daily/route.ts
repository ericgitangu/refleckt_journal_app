...
export async function GET() {
  ...
  try {
    ...
    try {
      const data = await serverApiClient('/prompts/daily', session.accessToken as string);
      return NextResponse.json(data);
    } catch (backendError) {
      // Log the error but don't fail - use mock data instead
      console.warn('Failed to fetch daily prompt from backend, using mock data:', backendError);
