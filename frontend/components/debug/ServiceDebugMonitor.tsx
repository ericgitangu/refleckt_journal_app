...
async function fetchServiceDebugInfo(serviceName: string): Promise<ServiceStatus> {
  try {
...
export async function ServiceDebugMonitor({ serviceName }: ServiceDebugMonitorProps) {
      ...
      <Card>
        <CardHeader className="pb-2">
          <div className="flex justify-between items-center">
            ...
            <Badge variant={
              serviceStatus.status === 'healthy' ? 'default' 
              : serviceStatus.status === 'degraded' ? 'secondary' 
              : 'destructive'
            }>
